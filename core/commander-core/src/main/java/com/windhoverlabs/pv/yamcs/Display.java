 ...
 *
 * <p>The numeric limits are given in double precision no matter which numeric type. The unit is a
 * simple String, which can be empty if no unit information is provided. The number format can be
 * used to convert the value to a String.
 */
public interface Display {
  ...
  /**
   * String representation of the units using for all values. Never null. If not available, returns
   * the empty String.
   */
  String getUnits();

  /**
   * Returns a NumberFormat that creates a String with just the value (no units). Format is locale
   * independent and should be used for all values (values and lower/upper limits). Never null.
   *
