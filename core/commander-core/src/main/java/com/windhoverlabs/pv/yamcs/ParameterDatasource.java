      ...
      switch (ptype.getEngType()) {
        ...
        case "integer[]":
          ...
          if (value instanceof int[]) {
            for (int intValue : (int[]) value) {
              intValueArray.addArrayValue(
                  Value.newBuilder().setType(Type.UINT64).setUint64Value(intValue));
            }
          } else {
            for (Object objectValue : (Object[]) value) {
              long longValue = Long.parseLong(String.valueOf(objectValue));
              intValueArray.addArrayValue(
                  Value.newBuilder().setType(Type.UINT64).setUint64Value(longValue));
            }
          }
        ...
        case "float[]":
          ...
          for (float floatValue : (float[]) value) {
            floatValueArray.addArrayValue(
                Value.newBuilder().setType(Type.FLOAT).setFloatValue(floatValue));
          }
        ...
        case "boolean[]":
          ...
          for (Object objectValue : (Object[]) value) {
            booleanValueArray.addArrayValue(
                Value.newBuilder()
