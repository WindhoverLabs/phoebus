...
public class ListNumbers {
   ...
   *
   * <p>The ListNumber is not sorted in place, and the data is not copied out. Therefore it's
   * intended that the ListNumber is not changed while the view is used.
   *
  ...
  /**
   * Creates a sorted view of the given ListNumber based on the indexes provided. This method can be
   * used to sort the given values based on the ordering by another (sorted) list of values.
   *
   * <p>The ListNumber is not sorted in place, and the data is not copied out. Therefore it's
   * intended that the ListNumber is not changed while the view is used.
   *
   ...
   *
   * <p>Note that, due to rounding errors in double precision, the difference between the elements
   ...
   * @return a new list
   */
  public static ListNumber linearListFromRange(
      final double minValue, final double maxValue, final int size) {
    if (size <= 0) {
  ...
  /**
   * Creates a list of equally spaced values given the first value, the step between element and the
   * size of the list.
   *
   ...
   * @return a new list
   */
  public static ListNumber linearList(
      final double initialValue, final double increment, final int size) {
    if (size <= 0) {
      throw new IllegalArgumentException("Size must be positive (was " + size + " )");
    }
   ...
   *
   * <p>Always returns true if the list was created with {@link #linearList(double, double, int) }
   * or {@link #linearListFromRange(double, double, int) }. For all other cases, takes the first and
   * last value, creates a linearListFromRange, and checks whether the difference is greater than
   * the precision allowed by double. Note that this method is really strict, and it may rule out
   * cases that may be considered to be linear.
   *
  ...
  public static boolean isLinear(ListNumber listNumber) {
    ...
    ListDouble diff =
        ListMath.subtract(
            listNumber,
            linearListFromRange(
                listNumber.getDouble(0),
                listNumber.getDouble(listNumber.size() - 1),
                listNumber.size()));
   ...
   *
   * @param primitiveArray must be an array of primitive numbers (byte[], short[], int[], long[],
   *     float[] or double[])
  ...
  public static ListNumber toListNumber(Object primitiveArray) {
    if (primitiveArray instanceof byte[]) {
      ...
      throw new IllegalArgumentException(
          primitiveArray + " is not a an array of primitive numbers");
    }
  }
