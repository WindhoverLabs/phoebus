...
public class CollectionNumbers {
  ...
  /**
   * If available, return the array wrapped by the collection - USE WITH CAUTION AS IT EXPOSES THE
   * INTERNAL STATE OF THE COLLECTION. This is provided in case an external routine for computation
   * requires you to use array, and you want to avoid the copy for performance reason.
   *
  ...
  /**
   * Returns either the wrapped array (if exists and matches the type) or a copy - USE WITH CAUTION
   * AS IT MAY EXPOSE THE INTERNAL STATE OF THE COLLECTION.
   *
   * @param coll the collection
   * @return the array
   */
