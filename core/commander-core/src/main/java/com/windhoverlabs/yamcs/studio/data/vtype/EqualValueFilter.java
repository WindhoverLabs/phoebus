...
public class EqualValueFilter {
  ...
  public boolean filterRow(int[] rowIndexes) {
    if (type == double.class) {
      double value =
          ((ListNumber) tables.get(0).getColumnData(columnIndexes[0])).getDouble(rowIndexes[0]);
      for (int i = 1; i < rowIndexes.length; i++) {
        if (value
            != ((ListNumber) tables.get(i).getColumnData(columnIndexes[i]))
                .getDouble(rowIndexes[i])) {
          return false;
        }
      }
    } else {
      ...
      for (int i = 1; i < rowIndexes.length; i++) {
        if (!Objects.equals(
            value, ((List) tables.get(i).getColumnData(columnIndexes[i])).get(rowIndexes[i]))) {
          return false;
        }
      }
    }
