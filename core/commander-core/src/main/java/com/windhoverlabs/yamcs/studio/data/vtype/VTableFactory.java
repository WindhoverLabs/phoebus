...
import static com.windhoverlabs.yamcs.studio.data.vtype.ValueFactory.alarmNone;
import static com.windhoverlabs.yamcs.studio.data.vtype.ValueFactory.displayNone;
import static com.windhoverlabs.yamcs.studio.data.vtype.ValueFactory.newVDoubleArray;
import static com.windhoverlabs.yamcs.studio.data.vtype.ValueFactory.newVStringArray;
import static com.windhoverlabs.yamcs.studio.data.vtype.ValueFactory.timeNow;
...
public class VTableFactory {
  ...
  public static VTable join(VTable... tables) {
    ...
    for (int nColumn = 0; nColumn < tables[0].getColumnCount(); nColumn++) {
      ...
      if (type.isPrimitive()) {
        ...
        columnData.add(
            ListNumbers.listView((ListNumber) tables[0].getColumnData(nColumn), rowIndexes.get(0)));
      } else {
        columnTypes.add(type);
      ...
      for (int nColumn = 0; nColumn < vTable.getColumnCount(); nColumn++) {
        if (!commonColumnsIndexes.containsKey(vTable.getColumnName(nColumn))) {
          ...
          if (type.isPrimitive()) {
            columnTypes.add(type);
            columnData.add(
                ListNumbers.listView(
                    (ListNumber) vTable.getColumnData(nColumn), rowIndexes.get(i)));
          } else {
            columnTypes.add(type);
  ...
  public static VTable union(
      VString extraColumnName, final VStringArray extraColumnData, final VTable... tables) {
    // Prune nulls
        ...
        if (size >= 0) {
          if (size != data.size()) {
            throw new IllegalArgumentException(
                "Column size does not match the others (this is "
                    + data.size()
                    + " previous is "
                    + size);
          }
  ...
  public static VTable valueTable(List<String> names, List<? extends VType> values) {
    ...
    throw new IllegalArgumentException(
        "Type "
            + ValueUtil.typeOf(values.get(0)).getSimpleName()
            + " not supported for value table");
  }
  ...
  private static VTable valueNumberTable(List<String> names, List<? extends VType> values) {
    ...
    if (names == null) {
      return newVTable(
          column(
              "Value",
              newVDoubleArray(new ArrayDouble(data), alarmNone(), timeNow(), displayNone())),
          column("Severity", newVStringArray(severity, alarmNone(), timeNow())),
          column("Status", newVStringArray(status, alarmNone(), timeNow())));
    } else {
      return newVTable(
          column("Name", newVStringArray(names, alarmNone(), timeNow())),
          column(
              "Value",
              newVDoubleArray(new ArrayDouble(data), alarmNone(), timeNow(), displayNone())),
          column("Severity", newVStringArray(severity, alarmNone(), timeNow())),
  ...
  public static void validateTable(VTable vTable) {
    for (int i = 0; i < vTable.getColumnCount(); i++) {
      ...
      if (String.class.equals(type)) {
        ...
        throw new IllegalArgumentException(
            "Column type " + type.getSimpleName() + " not supported");
      }
    }
  }
}
