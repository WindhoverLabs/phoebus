 ...
 *
 * <p>The factory methods do not do anything in terms of defensive copy and immutability to the
 * objects, which they are passed as they are. It's the client responsibility to prepare them
 * appropriately, which is automatically done anyway for all objects except collections.
 */
public class ValueFactory {
   ...
   * @return the new value
   */
  public static VMultiDouble newVMultiDouble(
      List<VDouble> values, final Alarm alarm, final Time time, final Display display) {
   ...
   * @return the new value
   */
  public static VLong newVLong(
      final Long value, final Alarm alarm, final Time time, final Display display) {
   ...
   * @return the new value
   */
  public static VInt newVInt(
      final Integer value, final Alarm alarm, final Time time, final Display display) {
   ...
   * @return the new value
   */
  public static VShort newVShort(
      final Short value, final Alarm alarm, final Time time, final Display display) {
   ...
   * @return the new value
   */
  public static VByte newVByte(
      final Byte value, final Alarm alarm, final Time time, final Display display) {
  ...
  private static final Display displayBoolean =
      newDisplay(0.0, 0.0, 0.0, "", NumberFormats.toStringFormat(), 1.0, 1.0, 1.0, 0.0, 1.0);
  ...
  public static Time newTime(
      final Instant timestamp, final Integer timeUserTag, final boolean timeValid) {
    return new Time() {
  ...
  public static Display newDisplay(
      final Double lowerDisplayLimit,
      final Double lowerAlarmLimit,
      final Double lowerWarningLimit,
      final String units,
      final NumberFormat numberFormat,
      final Double upperWarningLimit,
      final Double upperAlarmLimit,
      final Double upperDisplayLimit,
      final Double lowerCtrlLimit,
      final Double upperCtrlLimit) {
    return new Display() {
      @Override
  ...
  public static ArrayDimensionDisplay newDisplay(
      final ListNumber boundaries, final boolean reversed, final String unit) {
    return new IArrayDimensionDisplay(boundaries, reversed, unit);
  }

  public static ArrayDimensionDisplay newDisplay(
      final int size, final ListNumberProvider boundaryProvider, final boolean invert) {
    return newDisplay(boundaryProvider.createListNumber(size + 1), invert, "");
  }

  public static ArrayDimensionDisplay newDisplay(
      final int size, final ListNumberProvider boundaryProvider) {
  ...
  private static final Display displayNone =
      newDisplay(
          Double.NaN,
          Double.NaN,
          Double.NaN,
   ...
   * @return the new value
   */
  public static VDouble newVDouble(
      final Double value, final Alarm alarm, final Time time, final Display display) {
    return new IVDouble(value, alarm, time, display);
  }

  /**
   * Creates a new VDouble using the given value, time, display and generating the alarm from the
   * value and display information.
   *
  ...
  /**
   * Creates new immutable VDouble by using metadata from the old value, now as timestamp and
   * computing alarm from the metadata range.
   *
   ...
   * @return the new value
   */
  public static VFloat newVFloat(
      final Float value, final Alarm alarm, final Time time, final Display display) {
   ...
   * @return the new value
   */
  public static VStatistics newVStatistics(
   ...
   * @return a new value
   */
  public static VNumberArray newVNumberArray(
      final ListNumber data, final Alarm alarm, final Time time, final Display display) {
  ...
  public static VNumberArray newVNumberArray(
    ...
    throw new UnsupportedOperationException(
        "Data is of an unsupported type (" + data.getClass() + ")");
  }
  ...
  public static VNumberArray ndArray(VNumberArray data, ArrayDimensionDisplay... dimensions) {
    ...
    return ValueFactory.newVNumberArray(
        data.getData(), new ArrayInt(sizes), displays, data, data, data);
  }
   ...
   * @return the new value
   */
  public static VDoubleArray newVDoubleArray(
      ListDouble data, Alarm alarm, Time time, Display display) {
   ...
   * @return the new value
   */
  public static VFloatArray newVFloatArray(
      ListFloat data, Alarm alarm, Time time, Display display) {
   ...
   * @return a new object
   */
  public static VImage newVImage(
      int height,
      int width,
      final ListNumber data,
      VImageDataType imageDataType,
      Alarm alarm,
      Time time) {
   ...
   * @return the new value
   */
  public static VLongArray newVLongArray(
      final ListLong values, Alarm alarm, Time time, Display display) {
    ListInt sizes = new ArrayInt(values.size());
    return new IVLongArray(
        values, sizes, ValueUtil.defaultArrayDisplay(sizes), alarm, time, display);
  }
   ...
   * @return the new value
   */
  public static VIntArray newVIntArray(
      final ListInt values, Alarm alarm, Time time, Display display) {
   ...
   * @return the new value
   */
  public static VShortArray newVShortArray(
      final ListShort values, Alarm alarm, Time time, Display display) {
    ListInt sizes = new ArrayInt(values.size());
    return new IVShortArray(
        values, sizes, ValueUtil.defaultArrayDisplay(sizes), alarm, time, display);
  }
   ...
   * @return the new value
   */
  public static VByteArray newVByteArray(
      final ListByte values, Alarm alarm, Time time, Display display) {
    ListInt sizes = new ArrayInt(values.size());
    return new IVByteArray(
        values, sizes, ValueUtil.defaultArrayDisplay(sizes), alarm, time, display);
  }
   ...
   * @return the new value
   */
  public static VEnumArray newVEnumArray(
      ListInt indexes, List<String> labels, Alarm alarm, Time time) {
  ...
  /**
   * Takes a java objects and wraps it into a vType. All numbers are wrapped as VDouble. String is
   * wrapped as VString. double[] and ListDouble are wrapped as VDoubleArray. A List of String is
   * wrapped to a VStringArray. Alarms are alarmNone(), time are timeNow() and display are
   * displayNone();
   *
   ...
   * Takes a java objects and wraps it into a vType. All numbers are wrapped as VDouble. String is
   * wrapped as VString. double[] and ListDouble are wrapped as VDoubleArray. A List of String is
   * wrapped to a VStringArray. Alarms are alarm, time are timeNow() and display are displayNone();
   *
   * @param value the value to wrap
   * @param alarm the alarm for the value
   * @return the wrapped value
   * @deprecated use {@link #toVType(java.lang.Object, org.yamcs.studio.data.vtype.Alarm,
   *     org.yamcs.studio.data.vtype.Time, org.yamcs.studio.data.vtype.Display) }
   */
  @Deprecated
  public static VType wrapValue(Object value, Alarm alarm) {
    if (value instanceof Number) {
      ...
      throw new UnsupportedOperationException(
          "Type " + value.getClass().getName() + "  is not yet supported");
    }
  }

  /**
   * Converts a standard java type to VTypes. Returns null if no conversion is possible. Calls
   * {@link #toVType(java.lang.Object, org.yamcs.studio.data.vtype.Alarm,
   * org.yamcs.studio.data.vtype.Time, org.yamcs.studio.data.vtype.Display)} with no alarm, time now
   * and no display.
   *
   ...
   *
   * <p>Types are converted as follow:
   *
   * <ul>
   *   <li>Boolean -&gt; VBoolean
   *   <li>Number -&gt; corresponding VNumber
   *   <li>String -&gt; VString
   *   <li>number array -&gt; corresponding VNumberArray
   *   <li>ListNumber -&gt; corresponding VNumberArray
   *   <li>List -&gt; if all elements are String, VStringArray
   * </ul>
   *
   * @param javaObject the value to wrap
   * @param alarm the alarm
  ...
  /**
   * As {@link #toVType(java.lang.Object, org.yamcs.studio.data.vtype.Alarm,
   * org.yamcs.studio.data.vtype.Time, org.yamcs.studio.data.vtype.Display)} but throws an exception
