...
public class SimpleValueFormat extends ValueFormat {
  ...
  /**
   * Formats any scalar and array, by using the server side formatting and limiting the elements of
   * the array displayed to maxElements.
   *
  ...
  /**
   * Returns the appropriate NumberFormat: either the one from the data or the set by the formatting
   * options.
   *
  ...
  /**
   * Formats a string array. This method can be overridden to change the way string arrays are
   * formatted.
   *
  ...
  /**
   * Formats a boolean array. This method can be overridden to change the way string arrays are
   * formatted.
   *
   * @param data the data to format
   * @param toAppendTo the buffer to append to
  ...
  protected StringBuffer format(Array array, StringBuffer toAppendTo, FieldPosition pos) {
    ...
    throw new UnsupportedOperationException(
        "Type " + array.getClass().getName() + " not yet supported.");
  }
}
