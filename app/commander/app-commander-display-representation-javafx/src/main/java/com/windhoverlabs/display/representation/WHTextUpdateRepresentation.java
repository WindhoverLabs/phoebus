...
public class WHTextUpdateRepresentation
    extends RegionBaseRepresentation<Control, WHTextUpdateWidget> {
  ...
  @Override
  public Control createJFXNode()
      throws Exception { // Start out 'disconnected' until first value arrives
    System.out.println("WHTextUpdateRepresentation");
  ...
  protected void registerListeners() {
    ...
    pos =
        JFXUtil.computePos(
            model_widget.propHorizontalAlignment().getValue(),
  ...
  private void contentChanged(
    ...
    if (value_text.equals(new_text)) return;
    value_text = new_text;
    dirty_content.mark();
    toolkit.scheduleUpdate(this);
  }
  ...
  public void updateChanges() {
    ...
    if (dirty_style.checkAndClear()) {
      ...
      if (jfx_node.isManaged()) jfx_node.setPrefSize(w, h);
      else jfx_node.resize(w, h);

      if (model_widget.propTransparent().getValue()) jfx_node.setBackground(null); // No fill
      else {
        final Color color = JFXUtil.convert(model_widget.propBackgroundColor().getValue());
        jfx_node.setBackground(
    ...
    if (dirty_content.checkAndClear()) {
      if (jfx_node instanceof Label) ((Label) jfx_node).setText(value_text);
      else { // Implies 'interactive' mode
        final TextArea area = (TextArea) jfx_node;
      ...
      if (!jfx_node.isManaged()) jfx_node.layout();
    }
  }
}
