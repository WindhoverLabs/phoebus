...
public class WaypointCanvas extends Canvas {
  /** Area of this canvas */
  ...
  private void computeLayout(final Graphics2D gc, final Rectangle bounds) {
    ...
    if (scale_visible)
      plot_area.setBounds(
          bounds.x + scale_region.width,
          bounds.y + ends[1],
          bounds.width - scale_region.width,
          bounds.height - ends[0] - ends[1]);
    else plot_area.setBounds(bounds.x, bounds.y, bounds.width, bounds.height);
  }

  /** Draw all components into image buffer */
  protected Image updateImageBuffer() {
    final Rectangle area_copy = area;
    if (area_copy.width <= 0 || area_copy.height <= 0) return null;
    ...
    if (need_layout.getAndSet(false)) computeLayout(gc, area_copy);
    ...
    if (scale_visible) scale.paint(gc, plot_bounds);
    ...
    gc.setPaint(
        new GradientPaint(
            plot_bounds.x, 0, empty, plot_bounds.x + plot_bounds.width / 2, 0, empty_shadow, true));
    ...
    gc.setPaint(
        new GradientPaint(
            ...
            0,
            fill,
            plot_bounds.x + plot_bounds.width / 2,
            0,
            fill_highlight,
            true));
    if (normal)
