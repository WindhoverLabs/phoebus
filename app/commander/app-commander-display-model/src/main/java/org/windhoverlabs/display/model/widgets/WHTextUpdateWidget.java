...
import org.csstudio.display.builder.model.widgets.BorderSupport;
...
public class WHTextUpdateWidget extends PVWidget {
  /** Widget descriptor */
  public static final WidgetDescriptor WIDGET_DESCRIPTOR =
      new WidgetDescriptor(
          "windhover_textupdate",
          WidgetCategory.MONITOR,
          "Windhover Text Update",
  ...
  private static class CustomWidgetConfigurator extends WidgetConfigurator {
    ...
    public boolean configureFromXML(
      ...
      if (!super.configureFromXML(model_reader, widget, xml)) return false;
      if (xml_version.getMajor() < 2) {
        final WHTextUpdateWidget text_widget = (WHTextUpdateWidget) widget;
        WHTextUpdateWidget.readLegacyFormat(
            xml, text_widget.format, text_widget.precision, text_widget.propPVName());
        ...
        if (text.isPresent()
            && text.get().length() > 0
            && ((MacroizedWidgetProperty<String>) text_widget.propPVName())
                .getSpecification()
                .isEmpty()) {
          ...
          if (type != null && type.endsWith("TextUpdate") && pv_rule == false) {
            logger.log(
                Level.WARNING,
                "Replacing TextUpdate "
                    + text_widget
                    + " with 'text' but no 'pv_name' with a Label");
  ...
  static void readLegacyFormat(
      final Element xml,
      final WidgetProperty<FormatOption> format,
      final WidgetProperty<Integer> precision,
      final WidgetProperty<String> pv_name)
      throws Exception {
    XMLUtil.getChildInteger(xml, "format_type")
        .ifPresent(
            legacy_format -> {
    ...
    if (element != null && Boolean.parseBoolean(XMLUtil.getString(element))) precision.setValue(-1);
  ...
  protected void defineProperties(final List<WidgetProperty<?>> properties) {
    ...
    properties.add(
        horizontal_alignment =
            propHorizontalAlignment.createProperty(this, HorizontalAlignment.LEFT));
    properties.add(
        vertical_alignment = propVerticalAlignment.createProperty(this, VerticalAlignment.TOP));
  ...
  public WidgetProperty<?> getProperty(String name)
    ...
    if (name.equals("enabled")) return propVisible();
